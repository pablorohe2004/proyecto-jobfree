...
function MisServicios() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      try {
        perfil = await obtenerMiPerfil(token);
        perfilExistia = true;
      } catch {
      ...
      if (perfilExistia) {
        // Perfil ya existe → lo actualizamos con los nuevos datos
        await actualizarMiPerfil(token, perfil.id, datosPerfil);
      } else {
        // Primera vez → lo creamos
        perfil = await crearMiPerfil(token, datosPerfil);
      }
